class Worker::FavoritesController < ApplicationController
  ...
  def create
    ...
    if @favorite.save
      redirect_to employer_job_information_path(params[:job_information_id])

    end
  end
  def destroy
    @job_information = JobInformation.find_by(params[:job_information_id])
    @favorite = Favorite.where(nurse_id: current_nurse.id, job_information_id: params[:job_information_id])
    @favorite.destroy
    redirect_to employer_job_information_path(params[:job_information_id])
